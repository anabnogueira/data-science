...
def random_forest_feature_selection(data, n_estimators, max_depth, metric):
    nr_features = [0.3, 0.4, 0.5, 0.6, 0.7, 0.8, 0.9, 1.0]
    recall_values = []

    plt.figure()
    plt.title('Radom Forest and Feature Selection')
    plt.xlabel('Feature Selection')
    plt.ylabel(metric)
    ...
    for d in nr_features:
        ...
        recall = cross_val_score(rf, X, y, cv=5, scoring=metric)
        #print(nr_features)
        #print(recall.mean())
...
def knn(trnX, trnY, tstX, tstY):
    ...
    plt.figure()
    func.multiple_line_chart(plt.gca(), nvalues, values, 'KNN variations by number of neighbours', 'n', 'accuracy',
                             percentage=True)
    plt.show()
...
def knn_feature_selection(data):
    ...
    plt.figure()
    func.multiple_line_chart(plt.gca(), coefs, values, 'KNN variations by number of neighbours + CV', 'n', 'accuracy',
                             percentage=True)
    plt.show()
...
def clusters_plot(X):
    ...
    mbk = MiniBatchKMeans(init='k-means++', n_clusters=8, batch_size=45, n_init=10, max_no_improvement=10, verbose=0)
    t0 = time.time()
    mbk.fit(X)
